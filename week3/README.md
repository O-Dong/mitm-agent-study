...
평문 HTTP는 관찰뿐만 아니라 **변조까지 가능**하므로, 실제 프로덕션 환경에서는 반드시 **HTTPS (TLS/SSL 암호화)**를 사용해야 합니다.
